...
def test_add_major_minor_ticks():
    ...
    add_major_minor_ticks(
        ax, x_minor_per_major=4, y_minor_per_major=5, labelsize="small"
    )
    return fig
...
def test_find_local_extrema():
    ...
    data = xr.DataArray(
        data, dims=["lat", "lon"], coords=dict(lat=np.arange(4), lon=np.arange(5))
    )
...
def test_plot_contour_labels():
    ...
    plot_contour_labels(
        ax, p, ccrs.Geodetic(), proj, clabel_locations=contour_label_locations
    )
    return fig
